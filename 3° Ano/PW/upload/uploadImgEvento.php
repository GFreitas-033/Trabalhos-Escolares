...
if (isset($_GET['deletar'])) {
    ...
    $sql_query = $pdo->prepare("SELECT * FROM IMG_EVENTO WHERE id = :id");
    ...
    if ($arquivos && file_exists($arquivos['path']) && unlink($arquivos['path'])) {
        $deu_certo = $pdo->prepare("DELETE FROM IMG_EVENTO WHERE id = :id");
...
function enviarArquivo($error, $size, $name, $tmp_name) {
    ...
    if ($deu_certo) {
        $stmt = $pdo->prepare("INSERT INTO IMG_EVENTO (NOME, EVENTO_ID, PATH) VALUES (:nome, :evento_id, :path)");
        $stmt->execute([':nome' => $nomeDoArquivo, ':evento_id' => 1, ':path' => $path]);
        return true;
    } else {
        return false;
    }
...
$sql_query = $pdo->query("SELECT * FROM IMG_EVENTO");
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
